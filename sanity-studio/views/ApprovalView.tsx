...
import { Box, Button, Card, Flex, Spinner, Stack, Text, Badge, Heading, TextInput, Label } from '@sanity/ui'
import { useToast } from '@sanity/ui'

const API_BASE =
  process.env.SANITY_STUDIO_API_BASE_URL ?? 'https://aquamx-handoff.netlify.app'

type DocType = 'quotation' | 'contract'
...
export function ApprovalView({ document: doc, documentId }: Props) {
  ...
  const requestApproval = async (documentType: DocType) => {
    ...
    try {
      ...
      toast.push({
        ...
        description: data.message ?? 'Email sent to approver.',
        duration:    6000,
      })
    } catch (err: any) {
      toast.push({
  ...
  const cancelApproval = async (documentType: DocType) => {
    ...
    try {
      ...
      toast.push({
        ...
        description: 'You can now request approval again.',
        duration:    6000,
      })
    } catch (err: any) {
      toast.push({
    ...
    <Card padding={5} height="fill" overflow="auto">
      <Stack space={5}>
        ...
        <Card padding={4} radius={2} shadow={1}>
          <Stack space={3}>
            <Stack space={2}>
              ...
              <Text size={1} muted>
                Enter your email to receive the approved document automatically.
              ...
              requesting === 'quotation' ? (
                ...
                <Stack space={2}>
                  ...
                  <Button
                    ...
                    onClick={() => requestApproval('quotation')}
                  />
                </Stack>
              )
            )}
            ...
            {qStatus === 'pending' && (
              <Stack space={3}>
                <Text size={1} muted style={{ fontStyle: 'italic' }}>
                  Awaiting approver response. You will see the status update here once approved.
                </Text>
                {cancelling === 'quotation' ? (
                  ...
                  <Button
                    ...
                    onClick={() => cancelApproval('quotation')}
                  />
                )}
              </Stack>
            )}
        ...
        <Card padding={4} radius={2} shadow={1} tone={quotationApproved ? 'default' : 'transparent'}>
          <Stack space={4}>
            <Flex align="center" gap={3}>
              <Text weight="semibold" size={2} style={{ opacity: quotationApproved ? 1 : 0.45 }}>
                Step 2 — Contract Approval
              </Text>
              {quotationApproved
            ...
            {!quotationApproved && (
              <Text size={1} muted>Complete Step 1 first.</Text>
            )}
            ...
            {quotationApproved && (cStatus === 'not_requested' || cStatus === 'reset' || cStatus === 'rejected') && (
              requesting === 'contract' ? (
                ...
                <Stack space={2}>
                  ...
                  <Button
                    text="Request Contract Approval"
                    tone="primary"
                    disabled={!isPublished || requesting !== null || !emailValid}
                    onClick={() => requestApproval('contract')}
                  />
                </Stack>
              )
            )}
              ...
              <Stack space={3}>
                ...
                {cancelling === 'contract' ? (
                  ...
                  <Button
                    ...
                    mode="ghost"
                    disabled={requesting !== null || cancelling !== null}
                    onClick={() => cancelApproval('contract')}
                  />
                )}
              </Stack>
            )}
        ...
        <Text size={0} muted>
          Approval history is recorded in the Approval Requests section of the Studio.
        </Text>
